...
//    }
    ...
    @PutMapping("/update-profile/{riderId}")
    public ResponseEntity<?> updateProfile(@PathVariable Integer riderId, @RequestBody RiderUpdateDTO riderUpdateDTO) {
        riderService.updateProfile(riderId, riderUpdateDTO);
        return ResponseEntity.ok("Rider profile updated successfully");
    }

    @PutMapping("/update-drop-location/{riderId}")
    public ResponseEntity<?> updateDropLocation(@PathVariable Integer riderId, @RequestParam double dropLatitude, @RequestParam double dropLongitude) {
        riderService.updateDropLocation(riderId, dropLatitude, dropLongitude);
        return ResponseEntity.ok("Rider drop location updated successfully");
    }

    @PutMapping("/set-enabled/{riderId}")
    public ResponseEntity<?> setEnabled(@PathVariable Integer riderId, @RequestParam boolean enabled) {
        riderService.setEnabled(riderId, enabled);
        return ResponseEntity.ok("Rider enabled status updated successfully");
    }
